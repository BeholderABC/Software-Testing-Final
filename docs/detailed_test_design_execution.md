...
|---|---|---|---|---|
| REQ-009 | quantity less than stock | EP | TC-REQ-009-001 | `test_req009_quantity_equal_to_stock_is_accepted` |
| REQ-009 | quantity equals stock | BVA | TC-REQ-009 (boundary) | `test_req009_quantity_equal_to_stock_is_accepted` |
| REQ-009 | quantity greater than stock | EP / DT | TC-REQ-009 (negative) | `test_req009_quantity_one_above_stock_returns_400` |
| REQ-009 | quantity = 0 (lower bound) | BVA | added by interactive review | `test_req009_quantity_zero_returns_400` |

The final row is the lower-boundary case generated by the boundary
value technique; its outcome is reported in §6 and constitutes the
evidence underpinning Defect 1 in §7.
